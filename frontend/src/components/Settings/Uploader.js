...
const handleFileChange = (event) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response.data.mensagem === 'Archivo adjunto') {
        setUploaded(true);
    ...
    <Grid spacing={3} container>
      ...
      <form onSubmit={handleSubmit} className={classes.fullWidth}>
          ...
          <FormControl className={classes.selectContainer}>
            <InputLabel id="selectOption-label">Elija una opción:</InputLabel>
            <Select
        ...
        <Grid item xs={12} sm={12} md={12} style={{ display: 'flex' }}>
  			<FormControl className={classes.fullWidth}>
      			...
      			<input
                    ...
                    style={{ marginTop: 15, marginBottom: 15 }}
      			/>
      			{selectedFileName ? selectedFileName : 'Elija imagen PNG'}
    			</label>
  			</FormControl>
		</Grid>
